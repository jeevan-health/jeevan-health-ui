...
export default function AdminReports() {
  ...
  const handleMarkDelivered = (id, via) => {
    const all = loadReports().map(r => r.id === id ? { ...r, status: 'delivered', deliveredAt: new Date().toISOString(), deliveredVia: via } : r);
    saveReports(all);
    setReports(all);
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <TabBtn active={tab === 'all'} onClick={() => setTab('all')}>All ({reports.length})</TabBtn>
        <TabBtn active={tab === 'pending'} onClick={() => setTab('pending')}>Pending ({reports.filter(r => r.status === 'pending').length})</TabBtn>
        <TabBtn active={tab === 'approved'} onClick={() => setTab('approved')}>Approved ({reports.filter(r => r.status === 'approved').length})</TabBtn>
        <TabBtn active={tab === 'delivered'} onClick={() => setTab('delivered')}>Delivered ({reports.filter(r => r.status === 'delivered').length})</TabBtn>
        <div style={{ flex: 1 }} />
        <input placeholder="Search reports..." value={search} onChange={e => setSearch(e.target.value)} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, fontFamily: 'inherit', maxWidth: 240 }} />
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60, color: '#94a3b8', fontSize: 13 }}>No reports found</div>
      ) : (
          ...
          {filtered.map(r => (
            <div key={r.id} style={{ background: '#fff', borderRadius: 12, padding: 16, border: '1px solid #e2e8f0' }}>
              ...
              <div style={{ display: 'flex', gap: 16, fontSize: 11, color: '#94a3b8', marginTop: 8, flexWrap: 'wrap' }}>
                ...
                {r.verifiedAt && <span>Verified: {formatDate(r.verifiedAt)} by {r.verifiedBy}</span>}
                {r.deliveredAt && <span>Delivered: {formatDate(r.deliveredAt)} via {r.deliveredVia}</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowUpload(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, padding: 24, width: 440, maxWidth: '90vw' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h4 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#0f172a' }}>Upload Report</h4>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ fontSize: 12, color: '#64748b', display: 'block', marginBottom: 4 }}>Link to Order (optional)</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: '#64748b', display: 'block', marginBottom: 4 }}>Report File (PDF URL or upload)</label>
                <input placeholder="Paste file URL or filename" value={form.file} onChange={e => setForm({ ...form, file: e.target.value })} style={inputStyle} />
              </div>
            ...
            <div style={{ marginTop: 16, display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowUpload(false)} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit', color: '#64748b' }}>Cancel</button>
              <button onClick={handleUpload} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#0f172a', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit', color: '#fff', fontWeight: 600 }}>Upload Report</button>
            </div>
          </div>
        </div>
      )}
