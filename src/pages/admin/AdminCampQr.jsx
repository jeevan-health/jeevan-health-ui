...
/** Always patient site — never admin host (camp onboarding is for patients). */
const PATIENT_ORIGIN = 'https://jeevanhealthcare.com';
const CAMP_PATH = '/camp';
const JEEVAN_LOGO_URL = '/logo.png';
const SITE_DISPLAY = 'www.jeevanhealthcare.com';
const STORAGE_KEY = 'jh_camp_qr_card_v1';

const DEFAULT_HEADLINE = 'Welcome to Jeevan HealthCare';
const DEFAULT_TAGLINE = 'Scan to register · Get your lab report on email & app';
...
function loadImage(src) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    // Same-origin logo + data-URL partner logo — no cross-origin taint
...
/** Fit image into box preserving aspect ratio, centered. */
...
/**
 * Build a print-ready camp card PNG.
 * Layout: logos side-by-side → headline → tagline → QR → patient URL
 */
async function renderCampCardPng({
  campUrl,
  headline,
  tagline,
  companyLogoDataUrl,
  ...
  ctx.fill();
  // square bottom of bar so only top corners round visually
  ...
  try {
    ...
    // Fallback text logos if images fail
  ...
  ctx.textAlign = 'center';
  const headLines = wrapText(ctx, headline || DEFAULT_HEADLINE, W - pad * 2 - 40);
  ...
  const tagLines = wrapText(ctx, tagline || DEFAULT_TAGLINE, W - pad * 2 - 48);
  tagLines.forEach((ln) => {
    ctx.fillText(ln, W / 2, y);
    y += 26;
  });
  ...
  // QR (generated locally — no external host, no canvas taint)
  const qrSize = 340;
...
/**
 * Camp QR card builder for diagnostic camps.
 * Patients always land on patient site /camp — never admin host.
 */
export default function AdminCampQr() {
  const prefs = useMemo(() => loadPrefs(), []);
  const [headline, setHeadline] = useState(prefs?.headline || DEFAULT_HEADLINE);
  const [tagline, setTagline] = useState(prefs?.tagline || DEFAULT_TAGLINE);
  const [companyLogo, setCompanyLogo] = useState(prefs?.companyLogo || null);
  const [downloading, setDownloading] = useState(false);
  const [error, setError] = useState('');
  const [previewUrl, setPreviewUrl] = useState(null);
  const fileRef = useRef(null);

  const campUrl = `${PATIENT_ORIGIN}${CAMP_PATH}`;

  useEffect(() => {
    savePrefs({ headline, tagline, companyLogo });
  }, [headline, tagline, companyLogo]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = dataUrl;
      a.download = `jeevan-camp-qr-${new Date().toISOString().slice(0, 10)}.png`;
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        <div>
          <h2 style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 700 }}>📱 Camp QR card</h2>
          <p style={{ margin: 0, fontSize: 12, color: '#64748b', maxWidth: 520 }}>
            Build a print / booth card. QR always opens the <strong>patient</strong> site
            ({PATIENT_ORIGIN}/camp) — never the admin host. Download PNG to show at camps.
            After samples, upload PDFs under{' '}
            <Link to="/admin/lab-reports" style={{ color: '#1866C9' }}>Lab reports</Link>.
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button type="button" className="btn btn-primary" onClick={handleDownload} disabled={downloading || !previewUrl}>
            {downloading ? 'Preparing…' : '⬇️ Download card PNG'}
          </button>
          <Link to="/admin/lab-reports" className="btn btn-outline btn-sm">Upload reports →</Link>
        </div>
      </div>
      ...
      <div className="camp-qr-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 18 }}>
          ...
          <p style={{ margin: '0 0 8px', fontSize: 11, color: '#94a3b8' }}>
            Shown side-by-side with the Jeevan logo. Leave empty for Jeevan only.
          </p>
          ...
          {companyLogo && (
            <div style={{ marginTop: 10, padding: 10, background: '#F8FAFC', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
              <img src={companyLogo} alt="Company logo preview" style={{ height: 40, maxWidth: 120, objectFit: 'contain' }} />
          ...
          <div style={{ marginTop: 16, padding: 12, background: '#F0F7FF', borderRadius: 10, border: '1px solid #dbeafe' }}>
            ...
            <p style={{ margin: '6px 0 0', fontSize: 11, color: '#64748b' }}>
              Locked to patient portal. Admin URLs are never used here.
            </p>
          </div>
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', padding: 18 }}>
          ...
          <p style={{ margin: '10px 0 0', fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>
            Download the PNG and print / display on a tablet or poster at the booth.
          </p>
        </div>
      </div>
