...
/**
 * Real vaccination bookings from Neon via GET /admin/vaccination/bookings.
 * Legacy AdminVaccination.jsx remains for catalog/marketing mock tools.
 */
export default function AdminVaccinationBookings() {
  const t = useT();
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const { data } = await vaccinationService.adminListBookings({
      ...
      setError(err?.response?.data?.error || t('admin.vaccine.loadError', 'Failed to load vaccination bookings. Deploy API with /admin/vaccination routes.'));
      setBookings([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleStatus = async (b, status) => {
    setUpdatingId(b.id);
    try {
      await vaccinationService.adminUpdateBookingStatus(b.id, status);
      await load();
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 700 }}>{t('admin.vaccine.bookingsTitle', 'Vaccination bookings')}</h2>
          <p style={{ margin: 0, fontSize: 12, color: '#64748b' }}>
            {t('admin.vaccine.bookingsSub', 'Live Neon data. Catalog/CMS tools:')}{' '}
            <Link to="/admin/vaccination" style={{ color: '#1866C9' }}>{t('admin.vaccine.legacy', 'legacy Vaccination page')}</Link>
          </p>
        </div>
      ...
      {!loading && !error && bookings.length === 0 && (
        <div style={{ textAlign: 'center', padding: 48, background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', color: '#94a3b8', fontSize: 13 }}>
          No vaccination bookings yet. Patient flow: signed-in user completes /vaccination/book.
        </div>
      )}

      {!loading && bookings.length > 0 && (
        <>
          <div className="admin-vac-cards" style={{ display: 'none', flexDirection: 'column', gap: 10 }}>
            {bookings.map(b => (
              <div key={b.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14 }}>
                <div style={{ fontWeight: 700, marginBottom: 4 }}>#{b.id} · {b.vaccineName}</div>
                <div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.5, marginBottom: 8 }}>
                  ...
                  <div>₹{Number(b.totalAmount || 0).toLocaleString('en-IN')} · {b.serviceType || 'home'}</div>
                </div>
                <select
                  value={b.status}
          ...
          <div className="admin-vac-table" style={{ background: '#fff', borderRadius: 12, overflowX: 'auto', border: '1px solid #e2e8f0' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 720 }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  <th style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, color: '#64748b' }}>ID</th>
                  <th style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, color: '#64748b' }}>Vaccine</th>
                  ...
                  <th style={{ textAlign: 'left', padding: '10px 12px', fontSize: 11, color: '#64748b' }}>Type</th>
              ...
              <tbody>
                {bookings.map(b => (
                  <tr key={b.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '10px 12px' }}>{b.vaccineName}</td>
                    ...
                    <td style={{ padding: '10px 12px', textTransform: 'capitalize', fontSize: 12 }}>{b.serviceType || 'home'}</td>
      ...
      <style>{`
        @media (max-width: 768px) {
          .admin-vac-table { display: none !important; }
          .admin-vac-cards { display: flex !important; }
        }
      `}</style>
    </div>
  );
