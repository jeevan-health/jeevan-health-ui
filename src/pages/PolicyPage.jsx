  ...
  if (!policy) {
    return (
      <div style={{ textAlign: 'center', padding: '80px 16px' }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>{t('policy.notFound.title', 'Page Not Found')}</h1>
        <p style={{ fontSize: 13, color: '#64748b', marginBottom: 20 }}>{t('policy.notFound.desc', 'The policy page you are looking for does not exist.')}</p>
        <Link to="/" className="btn btn-primary">{t('policy.notFound.goHome', 'Go Home')}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="page-section container" style={{ maxWidth: MAX_WIDTH, margin: '0 auto', padding: '32px 16px' }}>
      <Link to="/" style={{ fontSize: 12, color: '#1866C9', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, marginBottom: 16 }}>
        {t('policy.homeLink', '← Home')}
      </Link>
      <div style={card}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#0f172a', margin: '0 0 4px' }}>{policy.title}</h1>
        <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 20px' }}>{t('policy.lastUpdated', 'Last updated:')} {policy.lastUpdated}</p>
