...
export default function NurseSchedule() {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>Your nursing shifts and patient visits</p>
      <div style={{ display: 'grid', gap: 8, gridTemplateColumns: 'repeat(auto-fit, minmax(90px, 1fr))' }}>
        {week.map((d, i) => (
          <div key={i} style={{ ...card, padding: 12, textAlign: 'center', marginBottom: 0, border: i === 0 ? '2px solid #0891b2' : '1px solid #e2e8f0' }}>
            ...
            <div style={{ fontSize: 10, color: i === 0 ? '#0891b2' : '#94a3b8' }}>{i === 0 ? 'Today' : ''}</div>
          </div>
        ))}
      </div>
      <div style={{ ...card, marginTop: 16 }}>
        <h4 style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', margin: '0 0 8px' }}>Today's Plan</h4>
        <div style={{ fontSize: 13, color: '#64748b' }}>Shift: 7:00 AM - 3:00 PM</div>
        <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>Patient visits scheduled: 3</div>
      </div>
    </div>
  );
}