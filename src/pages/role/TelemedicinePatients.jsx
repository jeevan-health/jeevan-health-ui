...
export default function TelemedicinePatients() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>{t('role.telemedicine.patients', '👤 Telemedicine Patients')}</h2>
      <input value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, width: 260, marginBottom: 16, fontSize: 12 }} placeholder={t('role.telemedicine.searchPatients', '🔍 Search patients...')} />
      {filtered.length === 0 && <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: 40 }}>{t('role.telemedicine.noPatients', 'No patients.')}</p>}
