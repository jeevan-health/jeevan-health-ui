...
export default function InventoryDashboard() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>{t('role.inventory.welcome', '📦 Welcome, ')}{user?.name || t('role.inventory.fallbackName', 'Inventory Staff')}</h2>
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', marginBottom: 16 }}>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0 }}><div style={{ fontSize: 24, fontWeight: 800, color: '#a16207' }}>{items.length}</div><div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{t('role.inventory.uniqueItems', 'Unique Items')}</div></div>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0 }}><div style={{ fontSize: 24, fontWeight: 800, color: '#a16207' }}>{totalStock.toLocaleString()}</div><div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{t('role.inventory.totalUnits', 'Total Units in Stock')}</div></div>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0 }}><div style={{ fontSize: 24, fontWeight: 800, color: '#a16207' }}>₹{totalValue.toLocaleString()}</div><div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{t('role.inventory.stockValue', 'Stock Value')}</div></div>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0, border: lowStock.length > 0 ? '2px solid #ef4444' : '1px solid #e2e8f0' }}>
          ...
          <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{t('role.inventory.lowStockAlerts', 'Low Stock Alerts')}</div>
        </div>
      </div>
      ...
      {lowStock.length > 0 && (
        <div style={{ ...card, border: '2px solid #fecaca', background: '#fef2f2' }}>
          <h4 style={{ fontSize: 13, fontWeight: 700, color: '#dc2626', margin: '0 0 10px' }}>{t('role.inventory.restock', '🚨 Items Need Restocking')}</h4>
          {lowStock.map(i => (
      ...
      {/* All Stock */}
      <div style={card}>
        <h4 style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', margin: '0 0 10px' }}>{t('role.inventory.allStock', '📋 All Stock')}</h4>
        {items.length === 0 && <p style={{ fontSize: 12, color: '#94a3b8' }}>{t('role.inventory.empty', 'No inventory items yet.')}</p>}
        {items.map(i => (
