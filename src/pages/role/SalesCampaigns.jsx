...
export default function SalesCampaigns() {
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>📢 Campaigns</h2>
      <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>Marketing campaigns and performance</p>
      {SAMPLE.map((c, i) => (
        <div key={i} style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ fontWeight: 600, fontSize: 14, color: '#0f172a' }}>{c.name}</span>
              <div style={{ fontSize: 12, color: '#64748b' }}>{c.channel} · Sent: {c.sent.toLocaleString()} · Opens: {c.opens} · Clicks: {c.clicks}</div>
              <div style={{ fontSize: 11, color: '#94a3b8' }}>Budget: {c.budget}</div>
            </div>
