...
export default function CallCenterBookings() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>{t('callCenterBookings.title', '📅 Bookings')}</h2>
      ...
      {filtered.map((b, i) => (
        <div key={b.id || i} style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ fontWeight: 600, fontSize: 14, color: '#0f172a' }}>{b.patientName || t('callCenterBookings.patient', 'Patient')}</span>
              <span style={{ fontSize: 11, color: '#64748b', marginLeft: 6 }}>{b.patientPhone}</span>
              <div style={{ fontSize: 12, color: '#64748b' }}>{b.testName || t('callCenterBookings.test', 'Test')} · {b.date} {b.time}</div>
            </div>
