...
export default function QADashboard() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>✅ Welcome, {user?.name || 'QA Officer'}</h2>
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', marginBottom: 16 }}>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0 }}><div style={{ fontSize: 24, fontWeight: 800, color: '#65a30d' }}>{qualityRate}%</div><div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>Quality Score</div></div>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0 }}><div style={{ fontSize: 24, fontWeight: 800, color: '#65a30d' }}>{completed.length}</div><div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>Completed Orders</div></div>
        <div style={{ ...card, padding: 16, textAlign: 'center', marginBottom: 0 }}><div style={{ fontSize: 24, fontWeight: 800, color: '#65a30d' }}>{cancelled.length}</div><div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>Cancelled / Issues</div></div>
      </div>
      <div style={card}>
        <h4 style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', margin: '0 0 10px' }}>📋 Recent Orders for Review</h4>
