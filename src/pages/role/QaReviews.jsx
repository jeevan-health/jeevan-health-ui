...
export default function QaReviews() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>🔍 Quality Reviews</h2>
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        {['', 'pending', 'approved', 'flagged'].map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{ padding: '4px 12px', borderRadius: 6, border: '1px solid #e2e8f0', background: filter === s ? '#65a30d' : '#fff', color: filter === s ? '#fff' : '#64748b', cursor: 'pointer', fontSize: 11 }}>{s || 'All'}</button>
        ))}
      </div>
      {filtered.map((r, i) => (
        <div key={i} style={{ ...card, borderLeft: `4px solid ${r.status === 'approved' ? '#65a30d' : r.status === 'flagged' ? '#dc2626' : '#f97316'}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: '#64748b' }}>By {r.reviewer} · {r.date}</div>
            </div>
