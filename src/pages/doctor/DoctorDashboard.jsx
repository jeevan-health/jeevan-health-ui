...
export default function DoctorDashboard() {
  ...
  return (
    <div>
      <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>👋 Dr. {user?.name || 'Doctor'}</h2>
      ...
      <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', marginBottom: 16 }}>
        {[
          { label: "Today's Appointments", value: grouped.todayApps.length, color: '#7c3aed', bg: '#f5f3ff' },
          { label: 'Upcoming', value: grouped.upcoming.length, color: '#2563eb', bg: '#eff6ff' },
          { label: 'Total Patients', value: uniquePatients, color: '#059669', bg: '#ecfdf5' },
          { label: 'Past Consultations', value: grouped.past.length, color: '#64748b', bg: '#f1f5f9' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 12 }}>
        {['today', 'upcoming', 'past'].map(t => (
          <button key={t} onClick={() => setView(t)} style={{
            ...
            background: view === t ? '#7c3aed' : '#f1f5f9',
            color: view === t ? '#fff' : '#475569',
            cursor: 'pointer', fontSize: 12, fontFamily: 'inherit', fontWeight: view === t ? 600 : 400,
          }}>{t === 'today' ? "Today's" : t === 'upcoming' ? 'Upcoming' : 'Past'}</button>
        ))}
      </div>

      {/* Appointments List */}
      <div style={sectionCard}>
        {view === 'today' && grouped.todayApps.length === 0 && <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: 24 }}>No appointments today.</p>}
        {view === 'upcoming' && grouped.upcoming.length === 0 && <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: 24 }}>No upcoming appointments.</p>}
        {view === 'past' && grouped.past.length === 0 && <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: 24 }}>No past consultations.</p>}
          ...
          <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid #f1f5f9' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: '#64748b' }}>
                {b.date} {b.time ? `@ ${b.time}` : ''} | {b.test || b.reason || 'Consultation'}
              </div>
