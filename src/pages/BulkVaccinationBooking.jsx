  ...
  if (submitted) {
    return (
      <div className="page-section container" style={{ maxWidth: 600, textAlign: 'center', paddingTop: 60 }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 800, margin: '0 0 8px', color: '#0f172a' }}>{t('request.submitted')}</h1>
        <p style={{ fontSize: 13, color: '#64748b', margin: '0 0 4px' }}>{t('bulk.request.received', 'Your bulk vaccination request has been received.')}</p>
        <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 24px' }}>{t('we.will.contact')}</p>
        <Link to="/vaccination" style={{ height: 44, padding: '0 24px', borderRadius: 8, border: 'none', background: '#2563eb', color: '#fff', fontSize: 13, fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}>{t('back.to.vaccination')}</Link>
      </div>
    );
  }
      ...
      <div style={{ background: 'linear-gradient(135deg, #059669 0%, #059669cc 100%)', padding: '28px 0 32px' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 6 }}>
            ...
            <div>
              <h1 style={{ color: '#fff', fontSize: 24, fontWeight: 700, margin: 0 }}>{t('bulk.vaccination')}</h1>
              <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13, margin: '2px 0 0' }}>{t('for.schools')}</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            {[1, 2, 3].map(s => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: 11, color: step >= s ? '#fff' : 'rgba(255,255,255,0.6)', fontWeight: step >= s ? 700 : 400 }}>{s === 1 ? t('organization') : s === 2 ? t('select.vaccines') : t('confirm.submit').replace('Confirm & ', '').replace(' & Submit', '')}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="page-section container" style={{ paddingTop: 20, maxWidth: 700 }}>
        {step === 1 && (
          <div style={{ padding: 20, borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 14px', color: '#0f172a' }}>{t('organization')}</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 14px' }}>
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('org.name', 'Organization / School / Society Name')} *</label>
              ...
              <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('contact.person')} *</label>
              ...
              <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('phone.number', 'Phone Number')} *</label>
              ...
              <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('email', 'Email')}</label>
              ...
              <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('city', 'City')} *</label>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('address', 'Address')}</label>
              ...
              {t('next.select.vaccines', 'Next → Select Vaccines')}
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ padding: 20, borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 4px', color: '#0f172a' }}>{t('select.vaccines')}</h3>
            <p style={{ fontSize: 11, color: '#64748b', margin: '0 0 12px' }}>{t('choose.vaccines.participants', 'Choose vaccines and specify participant count')} ({form.organization})</p>
            <div style={{ marginBottom: 10 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('estimated.participants', 'Estimated Participants')} *</label>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
              <button onClick={() => update('category', '')} style={{ padding: '4px 12px', borderRadius: 16, border: '1px solid #d0d5dd', background: !form.category ? '#059669' : '#fff', color: !form.category ? '#fff' : '#475569', fontSize: 10, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>{t('all', 'All')}</button>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              <button onClick={() => setStep(1)} style={{ padding: '8px 20px', borderRadius: 8, border: '1px solid #d0d5dd', background: '#fff', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>← {t('back', 'Back')}</button>
              <button onClick={() => setStep(3)} disabled={form.vaccineIds.length === 0}
                ...
                {t('next.confirm.submit', 'Next → Confirm & Submit')}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div style={{ padding: 20, borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 14px', color: '#0f172a' }}>{t('confirm.bulk', 'Confirm Bulk Booking')}</h3>
            ...
            <div style={{ marginBottom: 14 }}>
              <h4 style={{ fontSize: 12, fontWeight: 700, margin: '0 0 8px', color: '#0f172a' }}>{t('selected.vaccines', 'Selected Vaccines')} ({selectedVaccines.length})</h4>
            ...
            <div style={{ marginBottom: 14, padding: 12, borderRadius: 8, background: '#fffbeb', border: '1px solid #fef3c7' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 12, color: '#92400e', fontWeight: 600 }}>{t('estimated.total', 'Estimated Total')}</span>
                <span style={{ fontSize: 18, fontWeight: 800, color: '#059669' }}>₹{totalCost.toLocaleString()}</span>
              </div>
              <p style={{ fontSize: 10, color: '#92400e', margin: '4px 0 0' }}>{t('pricing.note', '* Final pricing may vary based on on-site assessment and group discounts')}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
              <div style={{ marginBottom: 0 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('preferred.date', 'Preferred Date')}</label>
              ...
              <div style={{ marginBottom: 0 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('preferred.time', 'Preferred Time')}</label>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ marginBottom: 0 }}>
                  <label style={{ fontSize: 11, fontWeight: 600, color: '#475569', display: 'block', marginBottom: 3 }}>{t('additional.notes', 'Additional Notes')}</label>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setStep(2)} style={{ padding: '8px 20px', borderRadius: 8, border: '1px solid #d0d5dd', background: '#fff', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>← {t('back', 'Back')}</button>
              <button onClick={handleSubmit} style={{ padding: '10px 32px', borderRadius: 8, border: 'none', background: '#059669', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>{t('submit.bulk', 'Submit Bulk Request')}</button>
            </div>
          </div>
        )}
      </div>
