    ...
    return (
      <div className="page-section">
        <div className="container" style={{ textAlign: 'center', padding: 60 }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700 }}>Doctor not found</h2>
          <Link to="/consult-doctor" style={{ color: '#1866C9' }}>← Back to all doctors</Link>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="page-section">
        <div className="container" style={{ textAlign: 'center', padding: 60 }}>
          ...
          <h2 style={{ fontSize: 24, fontWeight: 700, color: '#0f172a', margin: '0 0 8px' }}>Appointment Booked!</h2>
          <p style={{ fontSize: 14, color: '#64748b', maxWidth: 400, margin: '0 auto 20px', lineHeight: 1.6 }}>
            Your consultation with <strong>{doctor.name}</strong> has been scheduled for <strong>{f.date}</strong> at <strong>{f.time}</strong>.
            You will receive a confirmation call/message shortly.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <Link to="/consult-doctor" style={{ padding: '10px 24px', borderRadius: 8, border: '1px solid #d1d5db', background: '#fff', color: '#374151', textDecoration: 'none', fontSize: 14, fontFamily: 'inherit' }}>Book Another</Link>
            <Link to="/" style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: '#0f172a', color: '#fff', textDecoration: 'none', fontSize: 14, fontFamily: 'inherit', fontWeight: 600 }}>Go Home</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="page-section">
      <div className="container" style={{ maxWidth: 720 }}>
        <Link to="/consult-doctor" style={{ fontSize: 13, color: '#1866C9', textDecoration: 'none', display: 'inline-block', marginBottom: 16 }}>← Back to all doctors</Link>
        ...
        <div style={{ ...card, marginBottom: 24, display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: '#64748b' }}>{(doctor.specializations || []).join(', ')} | {(doctor.qualifications || []).join(', ')} | {doctor.experience} yrs</div>
            <div style={{ fontSize: 12, color: '#1866C9', fontWeight: 600, marginTop: 2 }}>₹{doctor.consultationFee || '—'}/consult</div>
          </div>
        </div>

        {/* Booking Form */}
        <div style={card}>
          <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f172a', margin: '0 0 20px' }}>Book an Appointment</h3>
          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 14 }}>
            <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
              <div>
                <label style={label}>Patient Name <span style={req}>*</span></label>
                <input style={input} value={f.patientName} onChange={e => update('patientName', e.target.value)} placeholder="Full name" required />
              </div>
              <div>
                <label style={label}>Patient Phone <span style={req}>*</span></label>
                <input style={input} type="tel" value={f.patientPhone} onChange={e => update('patientPhone', e.target.value)} placeholder="10-digit mobile" required />
              </div>
            </div>
            <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr 1fr' }}>
              <div>
                <label style={label}>Email</label>
                <input style={input} type="email" value={f.patientEmail} onChange={e => update('patientEmail', e.target.value)} placeholder="email@example.com" />
              </div>
              <div>
                <label style={label}>Age</label>
                <input style={input} type="number" value={f.patientAge} onChange={e => update('patientAge', e.target.value)} placeholder="Years" />
              </div>
              <div>
            ...
            {/* Consultation Mode */}
            <div>
              <label style={label}>Consultation Mode</label>
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                {['Online – Video', 'Online – Audio', 'Online – Chat', 'In-person Home Visit'].map(m => (
            ...
            <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
              ...
              <div>
                <label style={label}>Preferred Time <span style={req}>*</span></label>
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {timeSlots.map(t => (
            ...
            <div>
              ...
              <textarea style={{ ...input, resize: 'vertical', minHeight: 60 }} value={f.reason} onChange={e => update('reason', e.target.value)} placeholder="Brief description of your health concern..." />
            </div>

            {/* Address (for home visits) */}
            {f.mode === 'In-person Home Visit' && (
              <div>
                <label style={label}>Address for Home Visit <span style={req}>*</span></label>
                <textarea style={{ ...input, resize: 'vertical', minHeight: 60 }} value={f.address} onChange={e => update('address', e.target.value)} placeholder="Full address with landmark, city, pincode" required />
              </div>
            )}
