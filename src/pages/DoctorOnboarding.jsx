        ...
        <div style={card}>
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ fontSize: 13, color: '#374151', marginBottom: 12, lineHeight: 1.8 }}>
                ...
                <div>🏥 In-Clinic Consultations</div>
                <div>🚑 Home Visits</div>
              </div>
              <div style={{ background: '#f3f4f6', borderRadius: 10, padding: 16, fontSize: 13, color: '#374151', marginBottom: 16 }}>
                <strong>✅ Documents Required for Upload:</strong>
                <div style={{ marginTop: 6 }}>• 📄 Medical Degree Certificate</div>
                <div>• 🩺 Registration Certificate (MCI/TSMC)</div>
                <div>• 🏦 Bank Details (for payouts)</div>
              </div>
              <p style={{ fontSize: 12, color: '#64748b', fontStyle: 'italic', margin: '0 0 4px' }}>🔐 Your information will remain strictly confidential.</p>
              <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>⏱️ Form Completion Time: 5–7 minutes</p>
              <p style={{ fontSize: 12, color: '#94a3b8' }}>📞 Support: +91 9700104108 &nbsp;|&nbsp; 📧 care@jeevanhealthcare.com</p>
            </div>
          )}
            ...
            <div>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                <div>
                  ...
                  <input style={input} value={f.name} onChange={e => update('name', e.target.value)} placeholder="Dr. — Full name" />
                </div>
                <div>
                ...
                <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
                  ...
                  <div>
                    ...
                    <input style={input} type="tel" value={f.mobile} onChange={e => update('mobile', e.target.value)} placeholder="10-digit mobile" />
                  </div>
                </div>
                <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
                  <div>
          ...
          {step === 3 && (
            <div>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 16px' }}>Professional Details</h2>
              <div style={{ display: 'grid', gap: 14 }}>
                <div>
                  <label style={label}>Medical Qualification (e.g., MBBS, MD, DNB) <span style={req}>*</span></label>
                  <input style={input} value={f.qualification} onChange={e => update('qualification', e.target.value)} placeholder="e.g. MBBS, MD General Medicine" />
                </div>
                <div>
                  <label style={label}>Specialization <span style={req}>*</span></label>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 8 }}>
                    {SPECIALTIES.map(s => (
                      <button key={s} onClick={() => update('specialization', f.specialization === s ? '' : s)} style={chip(f.specialization === s)}>{s}</button>
                    ))}
                  </div>
                  <input style={input} value={f.specOther} onChange={e => update('specOther', e.target.value)} placeholder="Others (please specify)" />
                </div>
                <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr 1fr' }}>
                  <div>
                    <label style={label}>Years of Experience <span style={req}>*</span></label>
                    <input style={input} type="number" value={f.experience} onChange={e => update('experience', e.target.value)} placeholder="Years" />
                  </div>
                  <div>
                    <label style={label}>Medical Reg. Number <span style={req}>*</span></label>
                    <input style={input} value={f.regNumber} onChange={e => update('regNumber', e.target.value)} />
                  </div>
                  <div>
                    <label style={label}>Registration Council <span style={req}>*</span></label>
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                      {COUNCILS.map(c => (
                ...
                <div>
                  <label style={label}>Upload Degree/Certificate <span style={req}>*</span></label>
                  <input style={input} value={f.degreeUpload} onChange={e => update('degreeUpload', e.target.value)} placeholder="Paste link or file name" />
                </div>
                <div>
                  <label style={label}>Upload Registration Certificate <span style={req}>*</span></label>
                  <input style={input} value={f.regUpload} onChange={e => update('regUpload', e.target.value)} placeholder="Paste link or file name" />
                </div>
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 16px' }}>Availability & Consultation Mode</h2>
              <div style={{ display: 'grid', gap: 14 }}>
                <div>
                  <label style={label}>Preferred Consultation Modes <span style={req}>*</span></label>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {MODES.map(m => (
                ...
                <div>
                  <label style={label}>Available Days <span style={req}>*</span></label>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {DAYS.map(d => (
                ...
                <div>
                  <label style={label}>Available Time Slots <span style={req}>*</span></label>
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {SLOTS.map(s => (
                ...
                <div>
                  <label style={label}>Willing to do Home Visits?</label>
                ...
                <div>
                  <label style={label}>Preferred Areas for Home Visit</label>
                  <input style={input} value={f.homeAreas} onChange={e => update('homeAreas', e.target.value)} placeholder="e.g. ECIL, Begumpet, Gachibowli" />
                </div>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div>
              ...
              <div style={{ display: 'grid', gap: 14 }}>
                ...
                <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr' }}>
                  <div>
                    <label style={label}>IFSC Code <span style={req}>*</span></label>
                    <input style={input} value={f.ifsc} onChange={e => update('ifsc', e.target.value)} placeholder="IFSC code" />
                  </div>
                  <div>
                    <label style={label}>Upload Cancelled Cheque / Passbook <span style={req}>*</span></label>
