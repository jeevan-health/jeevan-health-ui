...
export default function UploadModal() {
  ...
  const validateFile = (f) => {
    ...
    if (!ACCEPTED.includes(f.type) && !f.name.match(/\.(jpg|jpeg|png|pdf)$/i)) return 'Unsupported format.';
    if (f.size > MAX_SIZE) return 'File exceeds 10 MB.';
    return '';
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!file) { alert('Upload your prescription first.'); return; }
    if (!form.name.trim() || !form.phone.trim()) { alert('Fill in name and phone.'); return; }
    setSubmitting(true);
        ...
        {step === 'upload' && (
          <div style={{ padding: 24 }}>
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Upload Prescription</h2>
              <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Upload your doctor's prescription and we'll recommend the right tests.</p>
            </div>
            <div onClick={() => inputRef.current?.click()} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); if (e.dataTransfer.files.length) handleFile(e.dataTransfer.files[0]); }}
              ...
              {file ? (
                ...
                <>
                  <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}><span style={{ color: 'var(--primary)' }}>Choose File</span> or drag & drop</p>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                ...
                <input type="tel" className="input" placeholder="10-digit phone number" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} required />
              </div>
              <div className="form-group">
                <label>Message <span style={{ fontSize: 11, color: 'var(--text-secondary)' }}>(Optional)</span></label>
                <textarea className="input" rows={2} placeholder="Any specific concerns..." value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} />
              </div>
        ...
        {step === 'success' && (
          <div style={{ padding: 32, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 13, lineHeight: 1.7, marginBottom: 20, textAlign: 'left', padding: '0 8px' }}>
              Our Health Advisor will review it and contact you shortly to recommend the appropriate tests and help schedule your home sample collection.
            </p>
            <button onClick={close} className="btn btn-primary btn-block">Done</button>
          </div>
        )}
      </div>
    </div>
