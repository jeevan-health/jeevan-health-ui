    ...
    <header className="site-header">
      <div className="header-inner">
        <button
          type="button"
          onClick={() => toggleMenu()}
          className={`hdr-hamburger${menuOpen ? ' is-open' : ''}`}
        ...
        <div className="hdr-right">
          ...
          <button onClick={() => useUploadModal.getState().setOpen(true)} className="hdr-btn hdr-upload" title={t('header.uploadTitle', 'Upload Prescription')}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
            <span className="hdr-label">{t('header.upload', 'Upload')}</span>
          </button>

          <a href="https://wa.me/919700104108" target="_blank" rel="noopener noreferrer" className="hdr-btn hdr-wa" title={t('header.whatsappTitle', 'WhatsApp')}>
          ...
          {isAuthenticated ? (
            ...
            <Link to="/signup" className="hdr-btn hdr-signup" title={t('header.signinTitle', 'Sign In')}>
              ...
              <span className="hdr-label">{t('header.login', 'Login')}</span>
            </Link>
          )}
        </div>
      </div>
      ...
      <style>{`
        ...
        .hdr-hamburger {
          display: none; background: none; border: none; padding: 8px; cursor: pointer; color: #333;
          border-radius: 10px; min-width: 40px; min-height: 40px; align-items: center; justify-content: center;
