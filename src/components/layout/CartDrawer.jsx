...
export default function CartDrawer() {
  ...
  const handleCoupon = () => {
    if (applyCoupon(couponVal)) { setCouponMsg(t('cartDrawer.couponApplied', 'Coupon applied!')); setCouponVal(''); }
    else setCouponMsg(t('cartDrawer.invalidCoupon', 'Invalid coupon code'));
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '100%', maxWidth: 420, background: '#fff', boxShadow: '-4px 0 24px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column', zIndex: 1, animation: 'cartSlideIn 0.25s ease' }}>
        <div style={{ padding: '14px 16px', borderBottom: '1px solid #e8edf2', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            ...
            <span style={{ fontSize: 15, fontWeight: 700 }}>{t('cartDrawer.yourCart', 'Your Cart')} ({items.length})</span>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '12px 16px' }}>
          {items.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-secondary)' }}>
              ...
              <p style={{ fontSize: 13, marginBottom: 12 }}>{t('cartDrawer.emptyCart', 'Your cart is empty')}</p>
              <Link to="/diagnostics" onClick={() => setCartOpen(false)} className="btn btn-primary">{t('cartDrawer.browseTests', 'Browse Tests')}</Link>
            </div>
          ) : (
            <>
              {items.map(i => {
                  ...
                  <div key={`${i.id}-${i.type}`} style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: '1px solid #f5f5f5' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 4 }}>
                        ...
                        {test?.fasting_required ? <span style={{ color: '#E65100' }}>🕐 {t('cartDrawer.fasting', 'Fasting')}</span> : <span style={{ color: '#16a34a' }}>✅ {t('cartDrawer.noFasting', 'No Fasting')}</span>}
              ...
              {recTests.length > 0 && (
                <div style={{ marginTop: 16, background: '#F5FAFF', borderRadius: 12, padding: '12px' }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#1866C9', marginBottom: 8 }}>⚡ {t('cartDrawer.freqAdded', 'Frequently Added Together')}</div>
                  {recTests.map(t => (
              ...
              <div style={{ marginTop: 16, background: '#FFF8E1', borderRadius: 12, padding: '12px' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#E65100', marginBottom: 8 }}>⭐ {t('cartDrawer.smartSavings', 'Smart Savings')}</div>
                <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginBottom: 6 }}>{t('cartDrawer.upgradeMsg', 'Upgrade to')} <strong>Executive Health Checkup</strong> — {t('cartDrawer.upgradeDesc', 'includes all selected tests + 18 more at a lower price.')}</div>
                <Link to="/services" onClick={() => setCartOpen(false)} style={{ fontSize: 11, fontWeight: 600, color: '#E65100', textDecoration: 'none' }}>{t('cartDrawer.viewPackage', 'View Package →')}</Link>
              </div>

              <div style={{ marginTop: 12, display: 'flex', gap: 6 }}>
                <input value={couponVal} onChange={e => setCouponVal(e.target.value.toUpperCase())} placeholder={t('cartDrawer.promoCode', 'Promo Code')} style={{ flex: 1, border: '1px solid #d0d5dd', borderRadius: 8, padding: '8px 10px', fontSize: 11, outline: 'none', fontFamily: 'inherit' }} />
                <button onClick={handleCoupon} style={{ padding: '8px 14px', background: 'var(--primary)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>{t('cartDrawer.apply', 'Apply')}</button>
              </div>
              {couponMsg && <div style={{ fontSize: 10, color: couponMsg === t('cartDrawer.couponApplied', 'Coupon applied!') ? '#16a34a' : '#dc2626', marginTop: 4 }}>{couponMsg}</div>}
            </>
          )}
        </div>
        ...
        {items.length > 0 && (
          <div style={{ borderTop: '1px solid #e8edf2', padding: '14px 16px', flexShrink: 0, background: '#fff' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}><span>{t('cartDrawer.subtotal', 'Subtotal')}</span><span>₹{subtotal.toLocaleString()}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4, color: '#16a34a' }}><span>{t('cartDrawer.homeCollection', 'Home Collection')}</span><span>{t('cartDrawer.free', 'FREE')}</span></div>
            {discAmt > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4, color: '#dc2626' }}><span>{t('cartDrawer.discount', 'Discount')} ({discount}%)</span><span>-₹{discAmt.toLocaleString()}</span></div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontWeight: 700, margin: '6px 0 10px', borderTop: '1px solid #e8edf2', paddingTop: 6 }}><span>{t('cartDrawer.total', 'Total')}</span><span>₹{total.toLocaleString()}</span></div>
            {discAmt > 0 && <div style={{ fontSize: 11, fontWeight: 600, color: '#16a34a', marginBottom: 8 }}>{t('cartDrawer.youSaved', '🎁 You Saved')} ₹{discAmt.toLocaleString()}</div>}
            <div style={{ display: 'flex', gap: 8 }}>
              <Link to="/diagnostics" onClick={() => setCartOpen(false)} className="btn btn-outline" style={{ flex: 1, fontSize: 12, padding: '10px', textAlign: 'center' }}>{t('cartDrawer.continueShopping', 'Continue Shopping')}</Link>
              <Link to="/checkout" onClick={() => setCartOpen(false)} className="btn btn-primary" style={{ flex: 1, fontSize: 12, padding: '10px', textAlign: 'center' }}>{t('cartDrawer.proceedToBook', 'Proceed to Book →')}</Link>
            </div>
          </div>
        )}
      </div>
