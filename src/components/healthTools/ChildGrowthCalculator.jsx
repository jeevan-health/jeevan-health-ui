  ...
  return (
    <div>
      ...
      <div style={s.inputFull}>
        <label style={s.label}>{t('child.headCircumference', 'Head Circumference (cm) — for infants under 3 years')}</label>
        <input className="input" type="number" step="0.1" value={form.headCircumference} onChange={e => u('headCircumference', e.target.value)} style={{ fontSize: 12 }} placeholder={t('child.optional', 'Optional')} />
      </div>

      {ageMonths !== null && <div style={{ fontSize: 11, color: '#64748b', marginBottom: 12 }}>{t('child.age', 'Age')}: {years}y {months}m</div>}

      <button style={s.btnCalc} onClick={() => setCalculated(true)} disabled={!canCalc}>👶 {t('child.calculateGrowth', 'Calculate Growth')}</button>

      {calculated && result && (
        <>
          <div style={{ textAlign: 'center', padding: '16px', background: '#f0fdf4', borderRadius: 12, marginBottom: 16, border: '1px solid #bbf7d0' }}>
            <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>{t('child.growthStatus', 'GROWTH STATUS')}</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: growthStatus()?.color || '#16a34a', marginTop: 4 }}>
              {growthStatus()?.icon} {growthStatus()?.label}
            </div>
            {form.name && <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>{t('child.for', 'for')} {form.name}</div>}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: result.headCircumference ? '1fr 1fr 1fr' : '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {result.heightPct !== null && (
              <div style={{ textAlign: 'center', padding: '12px 8px', background: '#f8fafc', borderRadius: 10 }}>
                <div style={{ fontSize: 10, color: '#64748b', fontWeight: 600 }}>{t('child.heightPercentile', 'Height Percentile')}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{result.heightPct}%</div>
              </div>
            )}
            {result.weightPct !== null && (
              <div style={{ textAlign: 'center', padding: '12px 8px', background: '#f8fafc', borderRadius: 10 }}>
                <div style={{ fontSize: 10, color: '#64748b', fontWeight: 600 }}>{t('child.weightPercentile', 'Weight Percentile')}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{result.weightPct}%</div>
              </div>
            )}
            {result.bmi !== null && (
              <div style={{ textAlign: 'center', padding: '12px 8px', background: '#f8fafc', borderRadius: 10 }}>
                <div style={{ fontSize: 10, color: '#64748b', fontWeight: 600 }}>{t('child.bmiForAge', 'BMI-for-Age')}</div>
          ...
          <div style={{ background: '#f0f9ff', borderRadius: 10, padding: 12, marginBottom: 16, border: '1px solid #bae6fd' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#0369a1', marginBottom: 6 }}>👶 {t('child.pediatricScreening', 'Pediatric Screening')}</div>
            <div style={{ fontSize: 11, color: '#0c4a6e', lineHeight: 1.6 }}>
              {t('child.pediatricDesc', 'Regular pediatric check-ups help monitor growth milestones and detect potential concerns early. Based on age, appropriate screening tests may be recommended by your pediatrician.')}
            </div>
          </div>

          {recs && recs.tests.length > 0 && (
            <div style={{ background: '#fdf8f0', borderRadius: 10, padding: 12, border: '1px solid #fde68a' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#92400e', marginBottom: 6 }}>🩺 {t('child.recommendedScreening', 'Recommended Screening')}</div>
              {recs.tests.map(t => (
              ...
              <button onClick={() => recs.tests.forEach(t => addItem({ id: t.name, type: 'test', name: t.name, price: t.price, offerPrice: t.price }))} style={{ ...s.btnSec, width: '100%', background: '#1866C9', color: '#fff', border: 'none', fontWeight: 600, marginTop: 8, marginBottom: 6 }}>
                🛒 {t('child.bookTests', 'Book Tests')} (₹{recs.totalCost})
              </button>
              <button onClick={() => window.location.href = '/contact'} style={{ ...s.btnSec, width: '100%', fontWeight: 600 }}>👨‍⚕️ {t('child.consultPediatrician', 'Consult Pediatrician')}</button>
            </div>
          )}
        </>
      )}

      {calculated && !canCalc && <div style={{ fontSize: 11, color: '#ef4444', marginBottom: 16 }}>{t('child.fillFields', 'Please fill in DOB, height, and weight to calculate growth.')}</div>}
    </div>
  );
}
