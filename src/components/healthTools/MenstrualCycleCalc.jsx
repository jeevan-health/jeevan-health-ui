  ...
  return (
    <div>
      <div style={s.inputRow3}>
        <div>
          <label style={s.label}>{t('menstrual.lastPeriod', 'Last Period Start')}</label>
          <input className="input" type="date" value={lmp} onChange={e => setLmp(e.target.value)} style={{ fontSize: 12 }} />
        </div>
        <div>
          <label style={s.label}>{t('menstrual.cycleLength', 'Cycle Length (days)')}</label>
        ...
        <div>
          <label style={s.label}>{t('menstrual.periodDuration', 'Period Duration')}</label>
      ...
      <button style={s.btnCalc} onClick={() => setCalculated(true)} disabled={!lmp}>📅 {t('menstrual.calculateCycle', 'Calculate Cycle')}</button>

      {calculated && cycle && (
        <div style={{ background: '#fdf2f8', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              { label: t('menstrual.nextPeriod', 'Next Period'), value: cycle.nextPeriod?.toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }), icon: '📅' },
              { label: t('menstrual.ovulationDay', 'Ovulation Day'), value: cycle.ovulation?.toLocaleDateString('en-IN', { day: 'numeric', month: 'long' }), icon: '🥚' },
              { label: t('menstrual.fertileWindow', 'Fertile Window'), value: `${cycle.fertileStart?.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })} – ${cycle.fertileEnd?.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}`, icon: '🌱' },
              { label: t('menstrual.pmsStart', 'PMS Start'), value: cycle.pmsStart?.toLocaleDateString('en-IN', { day: 'numeric', month: 'long' }), icon: '💫' },
            ].map(item => (
      ...
      {calculated && !lmp && <div style={{ fontSize: 11, color: '#ef4444', marginBottom: 16 }}>{t('menstrual.enterDate', 'Please enter your last period start date.')}</div>}

      {calculated && recs && recs.tests.length > 0 && (
        <div style={{ background: '#fdf8f0', borderRadius: 10, padding: 12, border: '1px solid #fde68a' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#92400e', marginBottom: 6 }}>🩺 {t('menstrual.recommendedScreening', 'Recommended Health Screening')}</div>
          ...
          <button onClick={() => recs.tests.forEach(t => addItem({ id: t.name, type: 'test', name: t.name, price: t.price, offerPrice: t.price }))} style={{ ...s.btnSec, width: '100%', background: '#1866C9', color: '#fff', border: 'none', fontWeight: 600, marginBottom: 6 }}>
            🛒 {t('menstrual.bookTests', 'Book Tests')} (₹{recs.totalCost})
          </button>
          <button onClick={() => window.location.href = '/contact'} style={{ ...s.btnSec, width: '100%', fontWeight: 600 }}>👨‍⚕️ {t('menstrual.consultGynaecologist', 'Consult Gynaecologist')}</button>
        </div>
      )}
    </div>
  );
