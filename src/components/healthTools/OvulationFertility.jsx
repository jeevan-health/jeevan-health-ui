  ...
  return (
    <div>
      <div style={s.inputRow}>
        <div>
          <label style={s.label}>{t('ovulation.lastPeriod', 'Last Period Start')}</label>
          <input className="input" type="date" value={lmp} onChange={e => setLmp(e.target.value)} style={{ fontSize: 12 }} />
        </div>
        <div>
          <label style={s.label}>{t('ovulation.cycleLength', 'Cycle Length (days)')}</label>
      ...
      <button style={s.btnCalc} onClick={() => setCalculated(true)} disabled={!lmp}>🌸 {t('ovulation.calculate', 'Calculate Ovulation')}</button>
        ...
        <div style={{ background: '#fdf2f8', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ padding: '8px 12px', background: '#fff', borderRadius: 10, textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: '#be185d', fontWeight: 600 }}>🥚 {t('ovulation.ovulationDay', 'Ovulation Day')}</div>
            ...
            <div style={{ padding: '8px 12px', background: '#fff', borderRadius: 10, textAlign: 'center' }}>
              <div style={{ fontSize: 10, color: '#be185d', fontWeight: 600 }}>🌱 {t('ovulation.fertileWindow', 'Fertile Window')}</div>
          ...
          <div style={{ marginTop: 12, padding: '8px 12px', background: '#fff', borderRadius: 10, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: '#be185d', fontWeight: 600 }}>📅 {t('ovulation.nextPeriodDue', 'Next Period Due')}</div>
      ...
      {calculated && cycle && (
        <div style={{ background: '#f0f9ff', borderRadius: 10, padding: 12, marginBottom: 16, fontSize: 11, color: '#0c4a6e', lineHeight: 1.6 }}>
          💡 {t('ovulation.fertileTip', 'Your fertile window is your best chance of conceiving. Ovulation typically occurs 14 days before your next period. For best results, aim to have intercourse during the fertile window. If you have been trying for 6+ months without success, consider consulting a fertility specialist.')}
        </div>
      )}

      {calculated && recs && recs.tests.length > 0 && (
        <div style={{ background: '#fdf8f0', borderRadius: 10, padding: 12, border: '1px solid #fde68a' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#92400e', marginBottom: 6 }}>🩺 {t('ovulation.preconceptionScreening', 'Preconception Health Screening')}</div>
          ...
          <button onClick={() => recs.tests.forEach(t => addItem({ id: t.name, type: 'test', name: t.name, price: t.price, offerPrice: t.price }))} style={{ ...s.btnSec, width: '100%', background: '#1866C9', color: '#fff', border: 'none', fontWeight: 600, marginBottom: 6 }}>
            🛒 {t('ovulation.bookTests', 'Book Tests')} (₹{recs.totalCost})
          </button>
        </div>
      )}
    </div>
