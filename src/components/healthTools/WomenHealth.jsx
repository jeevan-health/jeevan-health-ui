  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {[
          { key: 'cycle', label: '📅 Cycle Calculator', color: '#EC4899' },
          { key: 'pregnancy', label: '🤰 Due Date', color: '#8B5CF6' },
        ].map(t => (
      ...
      {tab === 'cycle' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
            <div>
              <label style={{ fontSize: 10, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Last Period Start</label>
              <input className="input" type="date" value={lmp} onChange={e => setLmp(e.target.value)} style={{ fontSize: 12 }} />
            </div>
            <div>
              <label style={{ fontSize: 10, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Cycle Length (days)</label>
            ...
            <div>
              <label style={{ fontSize: 10, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Period Duration (days)</label>
            ...
            <div style={{ background: '#fdf2f8', borderRadius: 12, padding: 16, marginBottom: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ...
                  { label: 'Ovulation', value: cycle.ovulation?.toLocaleDateString('en-IN', { day: 'numeric', month: 'long' }), icon: '🥚' },
      ...
      {tab === 'pregnancy' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
            <div>
              <label style={{ fontSize: 10, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Last Menstrual Period</label>
              <input className="input" type="date" value={lmp} onChange={e => setLmp(e.target.value)} style={{ fontSize: 12 }} />
            </div>
            <div>
              <label style={{ fontSize: 10, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Cycle Length (days)</label>
          ...
          {due && (
            <div style={{ background: '#f5f3ff', borderRadius: 12, padding: 16, marginBottom: 16 }}>
              <div style={{ textAlign: 'center', marginBottom: 12 }}>
                <div style={{ fontSize: 11, color: '#6d28d9', fontWeight: 600 }}>EXPECTED DELIVERY DATE</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div style={{ textAlign: 'center', padding: '8px', background: '#fff', borderRadius: 10 }}>
                  <div style={{ fontSize: 10, color: '#6d28d9' }}>Pregnancy Week</div>
                  <div style={{ fontSize: 20, fontWeight: 700, color: '#0f172a' }}>{due.weeks}w {due.days}d</div>
                </div>
                <div style={{ textAlign: 'center', padding: '8px', background: '#fff', borderRadius: 10 }}>
                  <div style={{ fontSize: 10, color: '#6d28d9' }}>Trimester</div>
          ...
          <div style={{ background: '#f0fdf4', borderRadius: 10, padding: 12, marginBottom: 16, border: '1px solid #bbf7d0' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#166534', marginBottom: 8 }}>🤰 Pregnancy Health Tracker</div>
            <div style={{ fontSize: 11, color: '#166534', lineHeight: 1.7 }}>
              <div>✅ Track weight gain, blood pressure, and blood sugar regularly</div>
              <div>✅ Monitor baby growth milestones during each trimester</div>
              <div>✅ Keep a record of all doctor visits and vaccinations</div>
              <div>✅ Schedule regular antenatal check-ups as recommended</div>
            </div>
          </div>

          {due && (
            <div style={{ background: '#fff7ed', borderRadius: 10, padding: 12, border: '1px solid #fed7aa' }}>
              <div style={{ fontSize: 11, color: '#9a3412', marginBottom: 4 }}>📋 Recommended Prenatal Tests</div>
              <div style={{ fontSize: 11, color: '#78350f', lineHeight: 1.6 }}>
                Based on your stage of pregnancy, consider: CBC, Blood Group & Rh, Blood Sugar, Thyroid Profile, Urine Routine, Vitamin D
              </div>
            </div>
          )}
        </>
      ...
      {recs && recs.tests.length > 0 && (
        <div style={{ background: '#fdf8f0', borderRadius: 10, padding: 12, border: '1px solid #fde68a', marginTop: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#92400e', marginBottom: 6 }}>🩺 Health Screening</div>
