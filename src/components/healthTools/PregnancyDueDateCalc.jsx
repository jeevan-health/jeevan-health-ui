...
export default function PregnancyDueDateCalc() {
  ...
  return (
    <div>
      ...
      <button style={s.btnCalc} onClick={handleCalculate} disabled={!lmp}>🤰 Calculate Due Date</button>
      ...
      {calculated && due && (
        <div style={{ background: '#f5f3ff', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 10, fontSize: 11, color: '#6d28d9' }}>
            Days Pregnant: {due.daysPregnant}
          </div>
        </div>
      )}

      {calculated && !lmp && <div style={{ fontSize: 11, color: '#ef4444', marginBottom: 16 }}>Please enter your last menstrual period date.</div>}

      {calculated && recs && recs.tests.length > 0 && (
        <div style={{ background: '#fff7ed', borderRadius: 10, padding: 12, border: '1px solid #fed7aa' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#9a3412', marginBottom: 6 }}>🩺 Recommended Prenatal Tests</div>
