    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {['cm', 'ft'].map(u => (
          ...
          }}>{u === 'cm' ? 'Centimeters' : 'Feet & Inches'}</button>
        ))}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div>
          <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>Height {unit === 'cm' ? '(cm)' : ''}</label>
          {unit === 'cm' ? (
            <input className="input" type="number" placeholder="e.g. 170" value={heightCm} onChange={e => setHeightCm(e.target.value)} style={{ fontSize: 13 }} />
          ) : (
            <div style={{ display: 'flex', gap: 6 }}>
              <input className="input" type="number" placeholder="Feet" value={feet} onChange={e => setFeet(e.target.value)} style={{ fontSize: 13, flex: 1 }} />
              <input className="input" type="number" placeholder="Inches" value={inches} onChange={e => setInches(e.target.value)} style={{ fontSize: 13, flex: 1 }} />
            </div>
          )}
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>Weight (kg)</label>
          <input className="input" type="number" placeholder="e.g. 72" value={weightKg} onChange={e => setWeightKg(e.target.value)} style={{ fontSize: 13 }} />
        </div>
      </div>
      ...
      {bmi && (
        <>
          <div style={{ textAlign: 'center', padding: '16px', background: '#f8fafc', borderRadius: 12, marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, marginBottom: 4 }}>YOUR BMI</div>
            ...
            <div style={{ fontSize: 11, color: '#64748b', marginTop: 8 }}>Healthy BMI Range: 18.5 – 24.9</div>
            {ideal && <div style={{ fontSize: 11, color: '#64748b' }}>Ideal Weight: {ideal.min} – {ideal.max} kg</div>}
          </div>

          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <button onClick={handleSave} style={{ ...btnStyle, background: '#1866C9', color: '#fff', border: 'none', flex: 1, fontWeight: 600 }}>💾 Save to History</button>
          </div>

          {/* History sparkline */}
          {bmiHistory.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#64748b', marginBottom: 6 }}>📊 BMI History</div>
          ...
          <div style={{ background: '#f0f9ff', borderRadius: 10, padding: 12, marginBottom: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#0f4a96', marginBottom: 6 }}>💡 Lifestyle Tips</div>
            <ul style={{ margin: 0, paddingLeft: 16, fontSize: 11, color: '#334155', lineHeight: 1.8 }}>
              {bmi >= 25 && <li>Walk 30 minutes daily to support weight management</li>}
              {bmi >= 25 && <li>Reduce processed foods and added sugars</li>}
              <li>Stay hydrated — aim for 8 glasses of water daily</li>
              <li>Sleep 7–8 hours for optimal health</li>
              {bmi < 18.5 && <li>Include protein-rich foods and healthy fats in your diet</li>}
            </ul>
          </div>
        </>
      )}
      ...
      {recs && recs.tests.length > 0 && (
        <div style={{ background: '#fdf8f0', borderRadius: 10, padding: 12, border: '1px solid #fde68a' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#92400e', marginBottom: 6 }}>🩺 Recommended for You</div>
          <p style={{ fontSize: 11, color: '#78350f', marginBottom: 8 }}>Based on your BMI, we recommend these preventive tests:</p>
          ...
          }}>🛒 Book Recommended Tests (₹{recs.totalCost})</button>
        </div>
      )}
    </div>
  );
