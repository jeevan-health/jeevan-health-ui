  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
        <div>
          <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>Systolic BP</label>
          <input className="input" type="number" placeholder="e.g. 120" value={systolic} onChange={e => setSystolic(e.target.value)} style={{ fontSize: 13 }} />
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>Diastolic BP</label>
          <input className="input" type="number" placeholder="e.g. 80" value={diastolic} onChange={e => setDiastolic(e.target.value)} style={{ fontSize: 13 }} />
        </div>
        <div>
          <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>Pulse (bpm)</label>
          <input className="input" type="number" placeholder="e.g. 72" value={pulse} onChange={e => setPulse(e.target.value)} style={{ fontSize: 13 }} />
        </div>
      </div>

      {systolic && diastolic && cat && (
        <div style={{ textAlign: 'center', padding: '16px', background: '#f8fafc', borderRadius: 12, marginBottom: 12 }}>
          <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, marginBottom: 4 }}>BLOOD PRESSURE</div>
          ...
          {pulse && <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>Pulse: {pulse} bpm</div>}
        </div>
      )}

      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button onClick={handleAdd} disabled={!systolic || !diastolic} style={{ ...btnStyle, background: '#1866C9', color: '#fff', border: 'none', flex: 1, fontWeight: 600, opacity: (!systolic || !diastolic) ? 0.5 : 1 }}>💾 Add Reading</button>
      </div>

      {/* History */}
      {bpReadings.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#64748b', marginBottom: 6 }}>📊 Reading History</div>
      ...
      <div style={{ background: '#f8fafc', borderRadius: 10, padding: 12, marginBottom: 16 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#64748b', marginBottom: 8 }}>📋 BP Classification</div>
        {[
          { label: 'Normal', range: '< 120 / < 80', color: '#16A34A' },
          { label: 'Elevated', range: '120–129 / < 80', color: '#F59E0B' },
          { label: 'Stage 1 HTN', range: '130–139 / 80–89', color: '#F97316' },
          { label: 'Stage 2 HTN', range: '≥ 140 / ≥ 90', color: '#EF4444' },
          { label: 'Crisis', range: '≥ 180 / ≥ 120', color: '#DC2626' },
        ].map(c => {
      ...
      {cat && cat.label !== 'Normal' && (
        <div style={{ background: '#fef2f2', borderRadius: 10, padding: 12, marginBottom: 16, border: '1px solid #fecaca' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: '#dc2626', marginBottom: 4 }}>⚠️ High BP Alert</div>
          <div style={{ fontSize: 11, color: '#7f1d1d', lineHeight: 1.5 }}>
            Your blood pressure is elevated. Please consult a healthcare provider for proper evaluation.
            Monitor your BP regularly and maintain a healthy lifestyle.
          </div>
        </div>
      )}
      ...
      {recs && recs.tests.length > 0 && (
        <div style={{ background: '#fdf8f0', borderRadius: 10, padding: 12, border: '1px solid #fde68a' }}>
          ...
          <button onClick={() => recs.tests.forEach(t => addItem({ id: t.name, type: 'test', name: t.name, price: t.price, offerPrice: t.price }))} style={{ ...btnStyle, width: '100%', background: '#1866C9', color: '#fff', border: 'none', fontWeight: 600 }}>
            🛒 Book Recommended Tests (₹{recs.totalCost})
          </button>
        </div>
      )}
    </div>
