      ...
      <div style={{ width: 220, background: '#fff', borderRight: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 20 }}>🩺</span>
              <span style={{ fontWeight: 700, fontSize: 14, color: '#0f172a' }}>{t('doctor.portal', 'Doctor Portal')}</span>
        </div>
        <nav style={{ flex: 1, padding: '8px 0' }}>
          {NAV.map(item => {
            const labels = { Dashboard: t('doctor.navDashboard', 'Dashboard'), Appointments: t('doctor.navAppointments', 'Appointments'), 'My Patients': t('doctor.navMyPatients', 'My Patients'), Availability: t('doctor.navAvailability', 'Availability') };
            return (
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #e2e8f0', background: '#fafbfc' }}>
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 8 }}>{user?.name || t('doctor.doctor', 'Doctor')}</div>
          <button onClick={() => { logout(); navigate('/'); }} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit', color: '#ef4444', width: '100%' }}>{t('doctor.logout', 'Logout')}</button>
        </div>
      </div>
      {/* Main */}
